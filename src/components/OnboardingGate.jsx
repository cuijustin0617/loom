...
import { loadSettings, saveSettings, isByokKeyFresh, API_KEY_MAX_AGE_DAYS } from '../utils/storage';
...
const OnboardingGate = ({ onComplete }) => {
  ...
  const needsKey = useMemo(() => !isByokKeyFresh(), [user]);
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      setSaving(true);
      saveSettings({ byokGeminiKey: trimmed });
      // Refresh freshness timestamp even if key value didn't change
