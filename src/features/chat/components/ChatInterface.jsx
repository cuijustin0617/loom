 ...
 * 
 * Handles message display, input, attachments, and streaming.
 */
...
function ChatInterface({ conversation }) {
  ...
  const messages = useMemo(() => {
    if (!conversation) {
      console.log('[ChatInterface] No conversation selected');
      return [];
    }
    console.log('[ChatInterface] Getting messages for conversation:', conversation.id);
    const msgs = Object.values(allMessages);
    const filtered = msgs
      .filter(m => m.conversationId === conversation.id)
      .sort((a, b) => a.createdAt.localeCompare(b.createdAt));
    console.log('[ChatInterface] Found', filtered.length, 'messages for', conversation.id);
    return filtered;
  ...
  }, [messages, selectedModel, autoScrollEnabled]);
  ...
  const readFilesAsAttachments = useCallback(async (files) => {
    const toRead = files.slice(0, 3); // Limit 3 files
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      console.error('[ChatInterface] File read error:', error);
    }
  };
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-white">
        <div className="text-center">
          <div className="text-2xl font-tech font-bold text-loom-blue mb-2">LOOM</div>
          <div className="text-gray-500">Start a new conversation to begin chatting</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 min-w-0 flex flex-col bg-white">
        ...
        {messages.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="w-full max-w-2xl px-6">
              <div className="mx-auto mb-4 h-14 w-14 rounded-xl border border-violet-300 bg-gradient-to-br from-violet-50 to-white flex items-center justify-center shadow-sm">
                <svg className="h-7 w-7 text-violet-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
              ...
              <form onSubmit={handleSubmit}>
                <div className={`flex flex-col gap-1 rounded-2xl border border-gray-300 bg-white shadow-sm px-4 pt-3 pb-2 ${isDragging ? 'ring-2 ring-violet-500 ring-offset-2' : ''}`}>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="opacity-80">
                        <ModelSelector compact selectedModel={selectedModel} onModelChange={setSelectedModel} />
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-[11px] text-gray-400 select-none">Cmd/Ctrl+Enter = send</div>
                      <button
                        type="submit"
                      ...
                      <div key={idx} className="group relative border border-violet-200 rounded-md p-2 bg-white shadow-sm">
                        {isImage ? (
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-700">
                            <svg className="h-5 w-5 text-violet-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
          ...
          <>
            {messages.map((message, index) => (
              <Message key={message.messageId || index} message={message} />
            ))}
      ...
      {/* Bottom input (only after conversation started) */}
      {messages.length > 0 && (
        <div className={`sticky bottom-0 z-10 border-t border-violet-200/70 bg-white/70 backdrop-blur px-3 sm:px-6 ${isDragging ? 'pt-6 pb-10' : 'pt-4 pb-6'}`}>
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-5xl mx-auto">
            <div className={`flex items-center gap-2 rounded-full border border-gray-300 bg-white shadow-sm px-3 sm:px-4 py-2 ${isDragging ? 'ring-2 ring-violet-500 ring-offset-2' : ''}`}>
              <button
                type="button"
            ...
            <div className="px-2 mt-1 text-[11px] text-gray-400 select-none">
              Cmd/Ctrl+Enter = send
            </div>
          </form>
        </div>
